...
// Create a throttle map to prevent duplicate requests
const throttleMap = new Map();
const THROTTLE_TIMEOUT = 2000; // 2 seconds
...
// Add request interceptor to add auth token and throttle requests
api.interceptors.request.use(
    (config) => {
        ...
        // Check if this request is being throttled
        if (throttleMap.has(requestKey)) {
            const now = Date.now();
            const lastTime = throttleMap.get(requestKey);

            if (now - lastTime < THROTTLE_TIMEOUT) {
                // Instead of canceling the request, use the cached response if available
                console.log(`Request throttled, using cache for: ${requestKey}`);

                // Let the request proceed, but update the cache timestamp
                throttleMap.set(requestKey, now);
                return config;
            }
        }

        // Update the throttle map with current timestamp
        throttleMap.set(requestKey, Date.now());
        return config;
    },
